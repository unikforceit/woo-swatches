...
class Term_Meta {
	 ...
	 * @static
	 *
	 * @var Emerce\VariationSwatches\Admin\Term_Meta
	 */
	 ...
	 * @static
	 *
	 * @return Emerce\VariationSwatches\Admin\Term_Meta An instance of the class.
	 */
	...
	public function get_swatches_types() {
		return [
			'color'  => esc_html__( 'Color', 'emerce-variation-swatches' ),
			'image'  => esc_html__( 'Image', 'emerce-variation-swatches' ),
			'label'  => esc_html__( 'Label', 'emerce-variation-swatches' ),
			'button' => esc_html__( 'Button', 'emerce-variation-swatches' ),
		];
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'emerce-variation-swataches-term', plugins_url( '/assets/css/admin.css', EMERCE_VARIATION_SWATCHES_FILE ), [ 'wp-color-picker' ], '20210630' );
		wp_enqueue_script( 'emerce-variation-swataches-term', plugins_url( '/assets/js/admin.js', EMERCE_VARIATION_SWATCHES_FILE ), [ 'jquery', 'wp-color-picker', 'wp-util', 'jquery-serialize-object' ], '20210630', true );
	}
	...
	public function add_attribute_fields( $taxonomy ) {
		...
		<div class="form-field term-swatches-wrap">
			...
			<p class="description"><?php esc_html_e( 'This data will be used for variation swatches of variable products.', 'emerce-variation-swatches' ) ?></p>
		</div>
	...
	public function field_label( $type ) {
		$labels = [
			'color'  => esc_html__( 'Swatches Color', 'emerce-variation-swatches' ),
			'image'  => esc_html__( 'Swatches Image', 'emerce-variation-swatches' ),
			'label'  => esc_html__( 'Swatches Label', 'emerce-variation-swatches' ),
		];
	...
	public function field_input( $type, $term = null ) {
		...
		switch ( $type ) {
			case 'image':
				...
				?>
				<div class="emerce-variation-swatches__field-image">
					...
					<p class="hide-if-no-js">
						<a href="#"
							...
							aria-label="<?php esc_attr_e( 'Swatches Image', 'emerce-variation-swatches' ) ?>"
							data-choose="<?php esc_attr_e( 'Use image', 'emerce-variation-swatches' ) ?>"
						>
							<?php esc_html_e( 'Upload', 'emerce-variation-swatches' ); ?>
						</a>
						<a href="#" class="button button-link button-remove-image <?php echo ! $value ? 'hidden' : '' ?>"><?php esc_html_e( 'Remove', 'emerce-variation-swatches' ) ?></a>
					</p>
			...
			case 'color':
				?>
				<p class="emerce-variation-swatches__field-color">
			...
			case 'label':
				?>
				<p class="emerce-variation-swatches__field-label">
	...
	protected function field_name( $type ) {
		return 'emerce_variation_swatches_' . $type;
	}
	...
	public function add_attribute_column_content( $columns, $column, $term_id ) {
		...
		switch ( $attr->attribute_type ) {
			case 'color':
				printf( '<div class="emerce-variation-swatches__thumbnail emerce-variation-swatches--color" style="background-color:%s;"></div>', esc_attr( $value ) );
				break;

			case 'image':
				$image = $value ? wp_get_attachment_image_url( $value ) : false;
				$image = $image ? $image : wc_placeholder_img_src( 'thumbnail' );
				printf( '<img class="emerce-variation-swatches__thumbnail emerce-variation-swatches--image" src="%s" width="40px" height="40px">', esc_url( $image ) );
				break;

			case 'label':
				printf( '<div class="emerce-variation-swatches__thumbnail emerce-variation-swatches--label">%s</div>', esc_html( $value ) );
				break;
		}
	}
