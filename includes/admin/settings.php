...
class Settings {
	const OPTION_NAME = 'emerce_variaiton_swatches';
	 ...
	 * @static
	 *
	 * @return Emerce\VariationSwatches\Admin\Settings An instance of the class.
	 */
	...
	public function get_shape_options() {
		$options = apply_filters( 'emerce_variation_swatches_shape_options', [
			'round'   => esc_html__( 'Round', 'emerce-variation-swatches' ),
			'rounded' => esc_html__( 'Rounded', 'emerce-variation-swatches' ),
			'square'  => esc_html__( 'Square', 'emerce-variation-swatches' ),
		] );
		...
		if ( current_theme_supports( 'woocommerce' ) ) {
			...
			if ( $theme_style ) {
				$options = array_merge( [ 'default' => esc_html__( 'Theme Default', 'emerce-variation-swatches' ) ], $options );
			}
		}
